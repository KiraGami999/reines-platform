...
export default async function PaymentReceiptPage({ params, searchParams }: PageProps) {
  ...
  const StatusIcon = isSuccess ? CheckCircle2 : isCancelled || payment.status === "FAILED" ? XCircle : Clock;
  const statusIconClass = isSuccess ? "text-blue-500" : isCancelled || payment.status === "FAILED" ? "text-blue-500" : "text-blue-500";
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      {/* Back */}
      <Link href="/dashboard/payments" className="inline-flex items-center gap-1.5 text-sm text-zinc-500 hover:text-zinc-800 transition-colors">
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 overflow-hidden shadow-sm">
        {/* Status header */}
        <div className={`px-8 py-8 text-center border-b border-zinc-100 ${isSuccess ? "bg-blue-50" : isPending ? "bg-blue-50" : "bg-blue-50"}`}>
          <StatusIcon size={48} className={`mx-auto mb-3 ${statusIconClass}`} />
        ...
        {/* Receipt details */}
        <div className="px-8 py-6 space-y-0 divide-y divide-zinc-100">
          {[
            { label: "Reference",   value: <span className="font-mono text-xs">{payment.txRef}</span> },
            { label: "Project",
              value: (
                <Link href={`/dashboard/projects/${payment.project.id}`} className="text-[#8fb9e8] hover:underline flex items-center gap-1">
                  {payment.project.title} <ExternalLink size={11} />
                </Link>
              )
            },
            { label: "Description",  value: payment.description ?? "—" },
            { label: "Amount",       value: <span className="font-bold text-lg">{fmtPaymentAmount(Number(payment.amount), payment.currency)}</span> },
            { label: "Currency",     value: payment.currency },
            { label: "Status",       value: <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-semibold ${meta.classes}`}><span className={`h-1.5 w-1.5 rounded-full ${meta.dot}`} />{meta.label}</span> },
            { label: "Paid at",      value: payment.paidAt ? fmtDate(payment.paidAt, true) : "—" },
            { label: "Initiated",    value: fmtDate(payment.createdAt, true) },
            ...(payment.paychanguId ? [{ label: "Paychangu ID", value: <span className="font-mono text-xs">{payment.paychanguId}</span> }] : []),
