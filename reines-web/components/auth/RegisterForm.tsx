    ...
    <div className="space-y-1.5">
      <div className="flex gap-1">
        {[0, 1, 2].map((i) => (
          <div
            key={i}
            className={`h-1 flex-1 rounded-full transition-colors ${i < score ? colour : "bg-zinc-100"}`}
          />
        ))}
      </div>
      <div className="flex items-center justify-between text-xs">
        ...
        {label && (
          <span className={`font-medium ${score === 3 ? "text-blue-600" : "text-zinc-400"}`}>
            {label}
          </span>
        )}
      </div>
    ...
    <form onSubmit={handleSubmit} className="space-y-4" noValidate>
      ...
      <div className="space-y-2">
        <div className="space-y-1">
          ...
          {fieldErrors.password && (
            <p className="text-xs text-blue-500">{fieldErrors.password}</p>
          )}
        </div>
      ...
      <div className="space-y-1">
        ...
        {fieldErrors.confirmPassword && (
          <p className="text-xs text-blue-500">{fieldErrors.confirmPassword}</p>
        )}
      </div>
