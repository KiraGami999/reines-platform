...
/** Returns true when sender changes OR >5 min gap between messages */
...
export default function ClientMessageThread() {
  const { projectId }     = useLocalSearchParams<{ projectId: string }>();
  const router            = useRouter();
  const { user }          = useAuth();
  const listRef           = useRef<FlatList<any>>(null);

  // Mark thread read on mount + unmount
  ...
  // Build flat list items: messages interspersed with date separators
      ...
      >
        ...
        {listItems.length === 0 ? (
          <View style={styles.centred}>
            ...
            <Text style={styles.emptySub}>Send the first message below.</Text>
          </View>
        ) : (
          <FlatList
            ref={listRef}
            ...
            renderItem={({ item, index }) => {
              ...
              // Previous *message* item (skip date separators)
              const prevMsg = listItems
                .slice(0, index)
                .reverse()
                .find((x) => x.type === "msg");
              const prevMessage = prevMsg?.type === "msg" ? prevMsg.msg : undefined;
              // Next *message* item
              const nextMsg = listItems
                .slice(index + 1)
                .find((x) => x.type === "msg");
